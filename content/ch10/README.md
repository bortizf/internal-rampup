...
elements of the DevOps Culture, which are the Continuous Integration/Continuous Delivery processes.
...
test it constantly every time a new feature is developed or even if a change is made on the code.

Another question rises up when we think what will happen once this application is finished. What do we do with it? how can we deliver it to the final client
...
make sure that the final product is not broken or has bugs?
...
Continuous Integration is the process responsible for being constantly testing your application. This requires you to push changes constantly, if you add a change to your code, Continuous Integration will build your application and test it, this way you'll make sure that everything is working ok. If something is broken, thanks to Continuous Integration you'll find that error faster. If you're not constantly testing, errors can appear once you deploy your application to production reducing the application's quality and worsening the final client experience. The faster you find an error, the faster you can fix it, and the better your application will be.

## What is Continuous Delivery (CD)?

Continuous Delivery is an extension of CI. After you have implemented your application's feature correctly and made sure that everything is working correctly testing constantly with every change you made, what should we do? That's right! we need to deploy our application!
...
this deployment is approved, you should be able to deploy with just one click to production environments.

## What is Continuous Deployment (CDP or CD)?

Continuous Deployment, also called CDP or CD is pretty similar to Continuous Delivery. The main difference here is that all the process is fully automated,
included deployments to production environments. This final step of deploying to production differ mostly because of business reasons, some business requires an approval for deploying to production and don't allow these deployments to be done automatically.

##

Those were the main purposes of CI/CD Processes. However, their implementation can differ among developers, you can add as many "middle" stages as you want.
...
help you understand the difference on these processes:
...
another one for the CD or you can have several for your CI, the implementation is yours and the sky is not the limit!
...
By exposing your pipelines, you can allow VCS tools to automatically run your CI pipeline, for instance each time you make a commit, ensuring that
you are constantly testing every change you made to your code and then, triggering automatically your CD pipeline, for instance each time a PR is approved,
...
* **Run your pipelines on different instances from the one your automation server is running**: Your automation server should be like a **main node**
which control **agent nodes** where the pipelines should run, this way you can create and delete these nodes on demand and won't hinder master node
performance.
...
webhook and send a trigger on every commit. This way you'll test every change developers do and allow them to identify errors faster.
...
* **Fail fast**: If there's something wrong with the application's code, your pipeline should fail as soon as possible, so developers know that
something is wrong and can start working on it as soon as possible.
...
talking about production. Also, you should add different notifications for certain situations, for example, if your instances exceeds 80% of
CPU usage.
